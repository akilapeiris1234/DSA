...
- **Usage**: Called via our Next.js API route (`/api/heart`) at the start of each new game
- **Response**: JSON with `solution` (hearts count) and `carrots` (carrots count)
- **Proxy**: Uses our own backend route to proxy the API response to bypass CORS errors.
